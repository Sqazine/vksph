...
#include <iostream>
VulkanDevice::VulkanDevice(const VulkanInstance *instance, std::vector<const char *> neededDeviceExtensions)
    : m_TmpInstanceHandle(instance), m_NeededDeviceExtensions(neededDeviceExtensions)
{
    ...
    m_GraphicsQueue = std::make_unique<VulkanQueue>(this, m_QueueIndices.graphicsFamily.value(), 0);
    m_PresentQueue = std::make_unique<VulkanQueue>(this, m_QueueIndices.presentFamily.value(), 0);
    m_ComputeQueue = std::make_unique<VulkanQueue>(this, m_QueueIndices.computeFamily.value(), 0);
}
...
const VkDevice &VulkanDevice::GetLogicalDeviceHandle() const
{
...
const VulkanQueue *VulkanDevice::GetPresentQueue() const
{
...
QueueFamilyIndices VulkanDevice::GetQueueIndices() const
{
