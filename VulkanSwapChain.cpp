...
#include <iostream>
VulkanSwapChain::VulkanSwapChain(SDL_Window *window, const VulkanInstance *instance, const VulkanDevice *device)
    : m_TmpVulkanDevice(device)
{
    ...
    VK_CHECK(vkCreateSwapchainKHR(m_TmpVulkanDevice->GetLogicalDeviceHandle(), &createInfo, nullptr, &m_SwapChainHandle));

    imageCount = 0;
    vkGetSwapchainImagesKHR(m_TmpVulkanDevice->GetLogicalDeviceHandle(), m_SwapChainHandle, &imageCount, nullptr);
    m_SwapChainImages.resize(imageCount);
    vkGetSwapchainImagesKHR(m_TmpVulkanDevice->GetLogicalDeviceHandle(), m_SwapChainHandle, &imageCount, m_SwapChainImages.data());
    ...
    for (size_t i = 0; i < m_SwapChainImages.size(); ++i)
    {
        ...
        VK_CHECK(vkCreateImageView(m_TmpVulkanDevice->GetLogicalDeviceHandle(), &info, nullptr, &m_SwapChainImageViews[i]));
    }
}
VulkanSwapChain::~VulkanSwapChain()
{
    for (auto imageView : m_SwapChainImageViews)
        vkDestroyImageView(m_TmpVulkanDevice->GetLogicalDeviceHandle(), imageView, nullptr);
